...
максимум 4 участника, у каждого не должно быть активной группы) — участникам уходит
инвайт с кнопками `grp_yes/grp_no`. Переход в `confirmed`, смена `users.status` и
уведомления происходят **только** в `checkGroupComplete()` (`bot.ts`), которую вызывает
callback бота; `POST /api/group/confirm` лишь ставит `ready=TRUE` и эту проверку не
запускает. Учитывать это при доработке подтверждения из Mini App.
